...
import { RAGNode, EnrichedNode, Habilidade, NivelEscolar } from "../types";
import { 
  detectarNivelEscolar, 
  validarHabilidadeNivel, 
  validarHabilidadeArea,
  extrairNumeroAno,
  validarHabilidadeAno
} from "./validators";
 ...
 */
export function filtrarNodesPorRelevancia(
  nodes: RAGNode[],
  tema: string,
  disciplina: string,
  serie: string,
  areaBNCC: string | null = null
): EnrichedNode[] {
  ...
  const [palavraNivelCorreto, palavrasNivelErrado] =
    nivelEscolar === "medio"
 ...
 */
export function extrairHabilidadesBNCC(
  nodes: EnrichedNode[], 
  nivelEscolar: NivelEscolar,
  areaBNCC: string | null = null,
  anoEsperado: number | null = null
): Habilidade[] {
  ...
  const habilidadesDescartadasNivel = todasHabilidades.filter(([codigo]) => 
    !validarHabilidadeNivel(codigo, nivelEscolar)
  );
  
  const habilidadesDescartadasArea = todasHabilidades.filter(([codigo]) => 
    validarHabilidadeNivel(codigo, nivelEscolar) && 
    areaBNCC && 
    !validarHabilidadeArea(codigo, areaBNCC)
  );
  
  const habilidadesDescartadasAno = todasHabilidades.filter(([codigo]) => 
    validarHabilidadeNivel(codigo, nivelEscolar) && 
 ...
 */
export function removerNodesDuplicados(nodes: EnrichedNode[], maxNodes: number = 10): EnrichedNode[] {
  const uniqueNodes: EnrichedNode[] = [];
